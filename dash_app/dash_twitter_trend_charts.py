...
app.layout = html.Div(children=[
    html.H2('Twitter Sentiment Trend Analysis', style={
        'textAlign': 'center'
    }),
    # html.H3("Currently tracking the keyword \"{}\" on Twitter.".format(TRACK_WORDS[0]), style={
    #     'textAlign': 'center'
    # }),
    html.Div(id='my-output'),
    # html.H4('(Last updated: Nov 16, 2020)', style={
    #     'textAlign': 'right'
    # }),
...
def retrieveKeyword():
    ...
    query = "select distinct keyword FROM {}".format(TABLE_NAME)
...
def update_header(n):
    # print('update_header called. input_value: ', n) 
    keyword = retrieveKeyword()
    # print('keyword 2', keyword)
    children = [html.H3("Currently tracking the keyword \"{}\" on Twitter.".format(keyword), style={
        'textAlign': 'center'
    })]
    #return '<h3>Output: {}</h3>'.format(keyword)
    return children
...
def update_graph_live(n):
    ...
    query = "SELECT id_str, text, created_at, polarity, user_location, user_followers_count FROM {}".format(TABLE_NAME)
    df = pd.read_sql(query, con=conn)


    # Convert UTC into PDT
    df['created_at'] = pd.to_datetime(df['created_at']).apply(lambda x: x - datetime.timedelta(hours=6))
    ...
    min10 = datetime.datetime.now() - datetime.timedelta(hours=6, minutes=10)
    min20 = datetime.datetime.now() - datetime.timedelta(hours=6, minutes=20)
    ...
    daily_impressions = back_up['impressions'].iloc[0] + df[df['created_at'] > (datetime.datetime.now() - datetime.timedelta(hours=6, seconds=10))]['user_followers_count'].sum()
    cur = conn.cursor()

    PDT_now = datetime.datetime.now() - datetime.timedelta(hours=6)
